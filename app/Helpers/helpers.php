...
function regen_url($key = '', $val = '', $action = '', $pageName = 'page')
{
...
// 数字转成中文
if (!function_exists('num2zh')) {
    function num2zh($num, $type)
    {
        ...
        $decimalZh = $decimal == '' ? '' : '点' . decimal4format($decimal, $type);
        $integerZh = do_trans($integer, $type, 1);
...
if (!function_exists('doTrans')) {
    function do_trans($number, $type, $isNeedUnit)
    {
        ...
        foreach ($formatNumberArray as $key => $val) {
            ...
            $str .= integer4format($val, $type, $isNeedUnit) . $unit;
        }
        return $str;
    }
}
// 四位为单位进行转换,带单位
if (!function_exists('integer4format')) {
    function integer4format($array, $type, $isNeedUnit = 1)
    {
        $zhSpecialArray = $type == 'chs' ? ['千', '百', '十', '零'] : ['仟', '佰', '拾', '零'];
        $zhPrimaryArray = get_data_by_type($type);
...
// 四位为单位进行转换,带单位
if (!function_exists('decimal4format')) {
    function decimal4format($array, $type)
    {
...
// 根据type 来获取数组
if (!function_exists('get_data_by_type')) {
...
function get_thumb_url($path = '', $width = null, $height = null, $cut = 0, $driver = null)
{
    return $path ? (new \App\Services\UploadService())->getThumbUrl($path, $width, $height, $cut, $driver) : '';
}

function get_upLoad_path($url)
{
    return $url ? \App\Services\UploadService::getUploadPath($url) : '';
}
